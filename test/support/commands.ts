...
Cypress.Commands.add('searchTravel', (searchParams: any) => {
  if (searchParams.destination) {
    cy.get('[data-testid="destination-select"]')
      .select(searchParams.destination)
  }
  
  if (searchParams.checkin) {
    cy.get('[data-testid="checkin-input"]')
      .type(searchParams.checkin)
  }
  
  if (searchParams.duration) {
    cy.get('[data-testid="duration-select"]')
      .select(searchParams.duration)
  }
  
  if (searchParams.guests) {
    cy.get('[data-testid="guests-select"]')
      .select(searchParams.guests)
  }
...
Cypress.Commands.add('likeDestination', (destinationName: string) => {
  cy.selectDestination(destinationName)
    .find('.like-btn')
    .click()
})
