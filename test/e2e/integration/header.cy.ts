...
describe("Header Component Tests", () => {
  ...
  describe(
    "Header Regional Hotline Link Tests",
    { tags: "checklist_integrated" },
    () => {
      it("should render each regional hotline phone number in the support dropdown as a clickable 'tel:' link with the correct phone number when the dropdown is open", () => {
        // Open the support dropdown
        cy.get('[data-testid="support-dropdown"]').click();

        // Define the regions to iterate through.  These regions and phone numbers MUST match what is in Header.tsx
        const regions = [
          { region: "Miền Bắc", regionId: "mien-bac", phone: "0909 009 009" },
          {
            region: "Miền Trung",
            regionId: "mien-trung",
            phone: "0909 007 007",
          },
          { region: "Miền Nam", regionId: "mien-nam", phone: "0909 080 808" },
        ];

        // Iterate through each region and assert the phone number link
        regions.forEach((region) => {
          const phoneSelector = `[data-testid="phone-${region.regionId}"]`;
          cy.get(phoneSelector)
            .should("exist")
            .should(
              "have.attr",
              "href",
              `tel:${region.phone.replace(/\s/g, "")}`
            );
        });
      });
    }
